...
yearPch = c('2021'=1, '2022'=2, '2023'=3)
```
...
```{r}
pmm = read.delim("../../data/from_ncat/PMMoV/PMMoV_SARS_2022-2023.txt") %>%
...
```{r}
# plot(x=pmm$ratio.SARStoPMMoV, y = (pmm$SARS.CoV.2.means/pmm$PMMoV.means)); abline(0,1)
# plot(x=pmm$ratio.SARStoPMMoV, y = (log10(pmm$SARS.CoV.2.means)/log10(pmm$PMMoV.means))); abline(0,1)
# plot(x=1:nrow(pmm), y = (pmm$SARS.CoV.2.means/pmm$PMMoV.means))
# plot(x=1:nrow(pmm), y = (log10(pmm$SARS.CoV.2.means)/log10(pmm$PMMoV.means)))
```
...
bonus question: is the variation less in PMM than SARS? ---not important question.


```{r}
# # Put both on a log scale.
# pmmPlot = pmm %>%
#     mutate(SARS.log10 = log10(SARS.CoV.2.means)) %>%
#     mutate(PMM.log10 = log10(PMMoV.means))
# 
# 
# # this post was very helpful: https://r-graph-gallery.com/line-chart-dual-Y-axis-ggplot2.html
# 
# # Value used to transform the data
# coeff = max(pmmPlot$SARS.log10) / max(pmmPlot$PMM.log10)
# 
# ggplot(pmmPlot, aes(x=date)) +
#   
#   geom_line( aes(y=SARS.log10), linewidth=2, color=sarsColor) + 
#   geom_line( aes(y=PMM.log10 / coeff), linewidth=2, color=pmmColor) +
#   
#   scale_y_continuous(
#     
...
#     sec.axis = sec_axis(~.*coeff, name="PMMoV (log10)")
#   ) + 
#   
#   theme_minimal() +
# 
#   theme(
#     axis.title.y = element_text(color = sarsColor, size=13),
#     axis.title.y.right = element_text(color = pmmColor, size=13)
#   ) +
# 
#   ggtitle("SARS and PMM")
...
```{r}
pmmD.2022 = read.delim("../../data/from_ncat/PMMoV/PMMoV_2022_by-dorm_2024-07-08.txt")
pmmD.2023 = read.delim("../../data/from_ncat/PMMoV/PMMoV_2023_by-dorm_2024-07-08.txt")
...
```{r}
rna.wide = read.delim("../../data/from_ncat/SARS-CoV-2_RNA-copies/RNA_copies_2021-2023.txt") %>%
    mutate(date = mdy(date, tz="")) 
rna = rna.wide %>% 
...
<!--        aes(x=Aggie.V4, y=Aggie.V4.AS, label=dateString, col=year)) + -->
<!--     scale_y_log10() + -->
<!--     scale_x_log10() + -->
<!--     scale_color_manual(values=yearColors) + -->
<!--     xlab("Grab Sampling") + -->
...
<!--     # annotate("text", x=1e16, y=1e17, label="unity", alpha=.3, position="dodge", angle=35) + -->
<!--     geom_point() + -->
...
<!--                           "\n", "N=", aV4testN)) + -->
<!--     theme_minimal() -->
<!-- ggsave("output/sampleMethod_AggieV4.png") -->
...
<!--     geom_abline(slope=1, intercept = 0, alpha=.2) + -->
<!--     # annotate("text", x=1e16, y=1e17, label="unity", alpha=.3, angle=35) + -->
<!--     geom_point() + -->
<!--     geom_text_repel(show.legend = F) + -->
...
<!--                           "\n", "N=", barbeeTestN)) + -->
<!--     theme_minimal() -->
<!-- ggsave("output/sampleMethod_Barbee.png") -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- ccd = read.delim("clincial-data-2021/output/reported-pos-tests_all_2021_byWeek.txt",  -->
<!--                  check.names = F) %>% -->
<!--     replace(is.na(.), 0) %>% -->
